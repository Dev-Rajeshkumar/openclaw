...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try { await registerUser({ email: data.email, password: data.password, fullName: data.fullName, businessName: data.businessName, phone: data.phone }); toast.success('Account created!'); router.push('/dashboard'); }
  ...
  const handleGoogleSuccess = async (idToken: string) => {
    ...
    catch (error: unknown) { toast.error(error instanceof Error ? error.message : 'Google signup failed'); }
  };
    ...
    <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <input id="fullName" type="text" placeholder="Rajesh Kumar" {...register('fullName')}
            className={cn('w-full px-4 py-3 rounded-xl border bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition', errors.fullName ? 'border-red-300' : 'border-gray-200')} />
        ...
        <div>
          <label htmlFor="businessName" className="block text-sm font-medium text-gray-700 mb-1">Business Name <span className="text-gray-400">(optional)</span></label>
      ...
      <GoogleLoginButton onSuccess={handleGoogleSuccess} onError={(error) => toast.error(error)} />
