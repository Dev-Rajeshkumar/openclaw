...
import {
  ...
  Menu, X, ChevronDown, Building2, Plus, Activity,
} from 'lucide-react';
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const handleBusinessSwitch = (business: IBusiness) => {
    setActiveBusiness(business);
    setBusinessMenuOpen(false);
    // Refresh the current page to reload data for new business
    router.refresh();
  };
        ...
        <div className="p-4 border-b border-gray-100">
          <div className="relative">
            <button
              ...
              className="w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl bg-amber-50 hover:bg-amber-100 transition"
            >
            ...
            {businessMenuOpen && (
              <>
                <div className="fixed inset-0 z-40" onClick={() => setBusinessMenuOpen(false)} />
                <div className="absolute left-0 right-0 mt-1 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-50 max-h-60 overflow-y-auto">
                  {businesses.map((b) => (
                    <button
                      key={b.id}
                      ...
                      className={cn(
                        ...
                        activeBusiness?.id === b.id && 'bg-amber-50 text-amber-700'
                      )}
                    >
                  ...
                  <div className="border-t border-gray-100 mt-1 pt-1">
                    <Link
                      ...
                      className="flex items-center gap-2 px-3 py-2 text-sm text-amber-600 hover:bg-amber-50 transition"
                    >
                      <Plus size={14} />
                      Add Business
                    </Link>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-gray-100">
          <div className="bg-gray-50 rounded-xl p-3">
            ...
            {activeBusiness?.plan === 'FREE' && (
              <Link href="/dashboard/settings" className="text-xs text-amber-600 hover:text-amber-700 font-medium mt-1 inline-block">
                Upgrade →
              </Link>
            )}
          </div>
