...
import { recurringInvoiceSchema, RecurringInvoiceFormData } from '@/lib/validations';
import { RecurringFrequency, IClient } from '@/types';
...
export default function NewRecurringPage() {
  ...
  const { register, handleSubmit, control, watch, setValue, formState: { errors } } = useForm<RecurringInvoiceFormData>({
  ...
  const onSubmit = async (formData: RecurringInvoiceFormData) => {
    ...
    try {
      ...
      const { data } = await api.post(`/clients/${formData.clientId}/recurring`, payload);
      if (data.success) { toast.success('Recurring invoice created!'); router.push('/dashboard/recurring'); }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-gray-900">New Recurring Invoice</h1><p className="text-gray-500">Set up automated billing</p></div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select onValueChange={(v) => setValue('frequency', v as RecurringFrequency)} defaultValue={RecurringFrequency.Monthly}>
            ...
            <div className="flex items-center gap-3">
              <Switch checked={watch('autoSend')} onCheckedChange={(v) => setValue('autoSend', v)} />
              <Label>Auto-send invoices to client email</Label>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            ...
            <div className="flex justify-end mt-4">
              <div className="w-64 space-y-2">
                <div className="flex justify-between font-bold border-t pt-2"><span>Subtotal</span><span className="text-amber-600">{formatCurrency(subtotal)}</span></div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent><Textarea placeholder="Notes included in each invoice..." {...register('notes')} rows={3} /></CardContent>
        </Card>

        <div className="flex justify-end gap-3">
          <Button type="button" variant="outline" onClick={() => router.back()}>Cancel</Button>
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? <><Loader2 size={18} className="animate-spin mr-2" />Creating...</> : 'Create Recurring Invoice'}
          </Button>
        </div>
      </form>
    </div>
