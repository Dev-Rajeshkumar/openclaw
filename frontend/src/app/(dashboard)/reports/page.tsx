      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <Card key={i}><CardContent className="p-4 md:p-5"><Skeleton className="h-4 w-20 mb-2" /><Skeleton className="h-7 w-24" /></CardContent></Card>
          ))}
        </div>
      ) : data ? (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <Card>
              <CardHeader><CardTitle>Export Data</CardTitle><CardDescription>Download reports as CSV</CardDescription></CardHeader>
              <CardContent className="space-y-2">
                {[
                  { type: 'invoices', label: 'Invoices', desc: 'All invoice data with line items' },
                  { type: 'clients', label: 'Clients', desc: 'Client list with contact details' },
